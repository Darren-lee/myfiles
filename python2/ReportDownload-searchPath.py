...
#parameters
try:
    options,args = getopt.getopt(sys.argv[1:], "hi:o:d:p:n:t:f:",["help","rsidFile=","path=","output=",
                                                                  "date=","name=","type=","file="])
...
projectListFile = str(mut_output) + "_"+type+"_Paths_all_" + str(filedate) + ".txt"
datapathFile = str(mut_output) + "_"+type+"_dataPath_" + str(filedate) + ".txt"
...
if pathfile == '':
    ...
    command = "cp " + pathfile + " " + projectListFile
    subprocess.call(command, shell=True)
